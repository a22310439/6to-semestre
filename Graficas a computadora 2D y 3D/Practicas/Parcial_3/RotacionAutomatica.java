...
public class RotacionAutomatica extends JFrame {
    ...
    // Ángulos de rotación del cubo alrededor de X, Y y Z (en radianes)
    private double angleX = 0.0;
    private double angleY = 0.0;
    private double angleZ = 0.0;

    // Eje actual de rotación: 'X', 'Y' o 'Z'
    private char currentAxis = 'Y';

    // Flag que indica si la rotación automática está activa
    private boolean autoRotate = false;

    public RotacionAutomatica() {
        super("Rotacion Automatica");
        setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
        getContentPane().setLayout(new java.awt.BorderLayout());

        // Instancia Graficos3D (sin cambios) y lo añade al JFrame
        graficos = new Graficos3D(this);

        setSize(width, height);
        setLocationRelativeTo(null);

        // Listener de teclado para alternar rotación y cambiar eje
        addKeyListener(new KeyAdapter() {
            @Override
            public void keyPressed(KeyEvent e) {
                switch (e.getKeyCode()) {
                    case KeyEvent.VK_R -> autoRotate = !autoRotate;
                    case KeyEvent.VK_X -> currentAxis = 'X';
                    case KeyEvent.VK_Y -> currentAxis = 'Y';
                    case KeyEvent.VK_Z -> currentAxis = 'Z';
                }
            }
        });
        ...
        // Timer para actualizar la rotación automática cada 10 ms
        int delay = 10; // ms
        new Timer(delay, ev -> {
            if (autoRotate) {
                double delta = Math.toRadians(2); // 2° por tick
                switch (currentAxis) {
                    case 'X' -> angleX += delta;
                    case 'Y' -> angleY += delta;
                    case 'Z' -> angleZ += delta;
    ...
    public static void main(String[] args) {
        SwingUtilities.invokeLater(RotacionAutomatica::new);
    }
}
