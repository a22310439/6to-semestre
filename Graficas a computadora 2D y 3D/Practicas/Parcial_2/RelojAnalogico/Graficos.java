...
public class Graficos {
    ...
    public Graficos(JFrame ventana) {
        ...
        try {
            backgroundImage = ImageIO.read(Graficos.class.getResource("background.jpg"));
            bgWidth  = backgroundImage.getWidth();
            bgHeight = backgroundImage.getHeight();
        } catch (IOException e) {
            backgroundImage = null;
            System.err.println("No se pudo cargar background.jpg: " + e.getMessage());
        }

        initSceneBuffer();

        canvasLabel = new JLabel(new ImageIcon(sceneBuffer));
        ventana.getContentPane().add(canvasLabel, BorderLayout.CENTER);
    ...
    private void initSceneBuffer() {
        ...
        int w = Math.max(d.width, 1), h = Math.max(d.height, 1);
    ...
    public void clear() {
        if (backgroundImage != null) {
            int w = sceneBuffer.getWidth(), h = sceneBuffer.getHeight();
            for (int y = 0; y < h; y++) {
                int srcY = y * bgHeight / h;
                for (int x = 0; x < w; x++) {
                    int srcX = x * bgWidth / w;
                    scenePixels[y * w + x] = backgroundImage.getRGB(srcX, srcY);
                }
            }
        } else {
            Arrays.fill(scenePixels, Color.WHITE.getRGB());
        }
    }
    ...
    public void drawTypeCircle(int xc, int yc, int r, Color c, int mask) {
        int x = 0, y = r, d = 3 - 2 * r, patternIndex = 0;
        while (x <= y) {
            if (((mask >> (patternIndex % 16)) & 1) == 1) {
                plot8(xc, yc, x, y, c);
            }
